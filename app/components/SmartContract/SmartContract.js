...
const SmartContract = ({ title = "", description = "", contractAddress, abi }) => {
  ...
  const { data: txList } = useSWR(
    `http://api-ropsten.etherscan.io/api?module=account&action=txlist&address=${contractAddress}&startblock=0&endblock=99999999&sort=asc&apikey=X8AMVJABPG8KEAZ5Z6A5VS19CB1WBZW7GT`,
    fetcher
  );
  ...
  return (
    <div className="container">
      <h1>{title}</h1>
      <p>{description}</p>
      <p>Interacting with <a target="_blank" href={`https://ropsten.etherscan.io/address/${contractAddress}`}>{contractAddress}</a></p>
